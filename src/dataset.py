...
def get_dataloaders(batch_size, train_path, test_path):
    ...
    train_input_sequences, train_output_sequences = create_pairs(train_velocities, train_positions)
    test_input_sequences, test_output_sequences = create_pairs(test_velocities, test_positions)
...
def create_time_pairs(velocities, positions, dt=0.1):
    ...
    output_sequences = np.hstack((velocities, positions))
    return input_sequences, output_sequences


def get_PINN_dataloaders(batch_size, train_path, test_path):
    train_velocities, train_positions = load_data(train_path)
    test_velocities, test_positions = load_data(test_path)
    
    train_input_sequences, train_output_sequences = create_time_pairs(train_velocities, train_positions)
    test_input_sequences, test_output_sequences = create_time_pairs(test_velocities, test_positions)
